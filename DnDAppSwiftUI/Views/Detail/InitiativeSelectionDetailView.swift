...
struct InitiativeSelectionDetailView: View {
    ...
    var body: some View {
        if let player {
            PlayerCharacterDetailView(player: player, encounterCombatent: combatent, allSpells: allSpells, onRollAbility: onRollAbility, onRollSkill: onRollSkill, onCastSpell: onCastSpell)
        } else if let monster {
